...
echoTest :: Spec
echoTest =
  around (SocketServer.with SocketServer.localConfig) $
...
multipleFnTest' =
  ...
  it "returns the original output for both" $ \s -> do
    let route = Route "echoInts"
        ...
        chars = ['a', 'b', 'c']
    SocketServer.serveRPC' s route (\() -> return $ Streamly.fromList xs)
    SocketServer.serveRPC' s routeCh (\() -> return $ Streamly.fromList chars)
    results <- SocketServer.callRPC' @() @Int clientConfig route ()
...
timeoutTest =
  ...
  it "forces callers to time out" $ \s -> do
    ...
    SocketServer.serveRPC s route (\() -> threadDelay (sec 0.2) >> return xs)
...
timeoutTest' =
  ...
  it "forces callers to time out" $ \s -> do
    ...
    (do results <-
          SocketServer.callRPCTimeout' @() @Int (sec 0.1) clientConfig route ()
